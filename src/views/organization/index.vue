          ...
          /> -->
            ...
            <el-form-item label="部门类型" class="require" v-if="setDepartmentList.length" prop="setDepartmentType">
                <el-checkbox-group v-model="setDepartmentType" @change="departTypeSelectChange">
                    <el-checkbox :label="item" v-for="(item,index) of setDepartmentList" :key="index" :disabled="false" style="display:inline-block">{{item.name}}</el-checkbox>
                </el-checkbox-group>
            </el-form-item>
            <div class="hideMod">
                <div class="wrap">
                                        ...
                                        <el-checkbox :label="item.eName" style="display:block;width:100%;height:100%">
                                            ...
                                            <div style="display:block">
                                                <el-checkbox-group v-model="childSelect" @change="childSelectChange($event,item)">
                                                    <el-checkbox v-for="(op,i) in item.childList" :key="i" :label="op.eName" style="display:block" :checked="checkPartmentTypeLevel2selectChild(item,op)">{{op.name}}</el-checkbox>
                                                </el-checkbox-group>
                                            </div>
                                        </el-checkbox>
                                    </el-checkbox-group>
        ...
        <el-form :model="addAndEditForm" label-width="100px">
            <template v-if="addAndEditForm.type=='add'||(addAndEditForm.editType&&addAndEditForm.editType==1)">
                ...
                <el-form-item label="登录密码：" class="require">
                    <el-input style="width:300px" v-model="addAndEditForm.password"></el-input>
                </el-form-item>
                <el-divider></el-divider>
            </template>

            <template v-if="addAndEditForm.type=='add'||(addAndEditForm.editType&&addAndEditForm.editType==2)">
                <h3 style="width:100%;text-align:right;font-size:14px;text-align:left;font-weight:normal">请选择该员工权限范围：<span style="color:#999">（该员工可以进入勾选的部门查看，具体权限由部门负责人设置）</span></h3>
                <el-form-item label="" class prop="phone" label-width="20px">
                    <el-checkbox-group v-model="addAndEditForm.departmentIds" @change="getdepartmentIds">
                        <el-checkbox :label="item.id" v-for="(item,index) in departmentList" :key="index">{{item.menuTitle}}</el-checkbox>
                    </el-checkbox-group>
                </el-form-item>
            </template>
        </el-form>
