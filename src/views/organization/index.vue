 ...
 * @LastEditTime: 2021-01-20 14:15:37
...
<template>
  <div class="organization" v-loading="loading">
    ...
    <div class="displayPart">
      <div class="warp">
          ...
          <div class="hdWrap">
            ...
            <div class="searchInfo">
              <div class="flex">
                ...
                <div>
                  <el-input @keyup.native="employees_list(activeLeftDepartMent.id)" v-model="employeesForm.content" prefix-icon="el-icon-search"></el-input>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="collapseWrap">
          ...
          <employees :employee="employeesList" @getDetails="getDetails" @getEmployeesDetailsEdit="getEmployeesDetailsEdit" @getEmployeesDelete="getEmployeesDelete"></employees>
        </div>
              ...
              <template v-for="(item,index) of setDepartmentList">
                <div class="cell" :key="index" v-show="departmentENameSelectCheck(item)">
                  <div class="cellWrap" style="display:block;width:100%;height:100%">
                    <el-checkbox-group v-model="selectAll" @change="addPartmentTypeLevel2All($event,item)">
                      <el-checkbox :label="item.eName" style="display:block;width:100%;height:100%">
                        <div>{{item.name}}</div>
                        <div style="display:block">
                          <el-checkbox-group v-model="childSelect" @change="childSelectChange($event,item)">
                            <el-checkbox v-for="(op,i) in item.childList" :key="i" :label="op.eName" style="display:block" :checked="checkPartmentTypeLevel2selectChild(item,op)">
                              {{op.name}}
                            </el-checkbox>
                          </el-checkbox-group>
                        </div>
      ...
      <div class="iconAndColorDia">
        <div class="row">
          ...
          <div class="pool clearfix">
            <ul>
              <li v-for="(item,index) of iconList" :key="index" class="iconPoolLi" @click="iconSelectFn(item)" :class="{on:selectIcon==item.eName}">
                <img :src="item.name" />
              </li>
            </ul>
          </div>
        ...
        <div class="row">
          ...
          <div class="pool clearfix">
            <ul>
              <li class="colorPoolLi" v-for="(item,index) of colorList" :key="index" :style="{backgroundColor:'#'+item.name}" @click="selectColor=item.name" :class="{on:selectColor==item.name}">
    ...
    <el-dialog top="0" :visible.sync="employeesDetailsEditShow" :title="addAndEditForm.type=='add'?this.$t('boss.add_addMembers'): (addAndEditForm.editType == 2 ? this.$t('boss.add_editorDepart') : this.$t('boss.add_editorEmpe'))" width="600px" @close="itemClose()">
      <el-form :model="addAndEditForm" label-width="100px" size="small">
        <template v-if="addAndEditForm.type=='add'||(addAndEditForm.editType && addAndEditForm.editType==1)">
          ...
          <el-form-item :label="$t('boss.add_loginPass')+':'" class="require">
            <el-input style="width:300px" v-model="addAndEditForm.password" type="password" @input="passwordChange"></el-input>
          </el-form-item>
          ...
          <el-form-item label="" class prop="phone" label-width="20px">
            <el-checkbox-group v-model="addAndEditForm.departmentIds" @change="handleCheckedDeptsChange">
              <el-checkbox :label="item.id" v-for="(item,index) in departmentList" :key="index">
                {{ $i18n.locale == 'ri' ? item.japanese : item.menuTitle }}
              </el-checkbox>
            </el-checkbox-group>
          </el-form-item>
        </template>
...
export default {
  ...
  methods: {
    ...
    enterprise_department_distributionFn(item) {
      ...
      enterprise_department_distribution(params)
        .then(async (res) => {
          if (res.code == 200) {
            params.roleStatus = 1;
    ...
    getEmployeesDelete(item) {
      ...
      this.$F.doRequest(
        this,
        "/pms/workuser/delete_login_user",
        params,
        (res) => {
          this.employees_list(this.activeLeftDepartMent.id);
          this.$forceUpdate();
    ...
    fetchStoresUserCount() {
      this.$F.doRequest(
        this,
        "/pms/workuser/stores_user_count",
        { departmentId: "3213" },
        (res) => {
          ...
          .then((res) => {
            if (res.code == 200) {
              ...
              for (let k in res.data.departmentMenuList) {
                let obj = {
                  departmentEName:
                    res.data.departmentMenuList[k].departmentTypeId,
                ...
                if (setDepartmentListNum[obj.departmentEName]) {
                  setDepartmentListNum[
                    obj.departmentEName
                  ] = ++setDepartmentListNum[obj.departmentEName];
                } else {
