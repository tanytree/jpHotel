...
<template>
    <div class="finance">
        ...
        <el-dialog top="0" :title="$t('desk.enterAccount')" :visible.sync="entryShow">
            ...
            >
                <el-form-item  :label="$t('desk.order_payProject')+':'">
                    <el-radio-group v-model="consumeOperForm.priceType">
                        <el-radio-button :label="3" :value="3"
                            >{{$t('desk.customer_collection')}}</el-radio-button
                        >
                        <el-radio-button :label="2" :value="2"
                            >{{$t('desk.order_theDeposit')}}</el-radio-button
                        >
                    </el-radio-group>
                </el-form-item>

                <el-form-item
                    :label="$t('desk.order_selectPayWay')"
                    v-if="
                        consumeOperForm.priceType == 3 ||
                        consumeOperForm.priceType == 2
                    "
                >
                ...
                <el-form-item :label="$t('desk.order_consumptionProject')+':'">
                    <el-radio-group
                        v-model="consumeOperForm.priceType"
                        @change="priceTypeChange"
                    >
                        <el-radio-button :label="5" :value="5"
                            >{{$t('desk.order_addDayPrice')}}</el-radio-button
                        >
                        <el-radio-button :label="6" :value="6"
                            >{{$t('desk.order_addHalfPrice')}}</el-radio-button
                        >
                        <el-radio-button :label="7" :value="7"
                            >{{$t('desk.order_loosAndCompensation')}}</el-radio-button
                        >
                    </el-radio-group>
                </el-form-item>
                ...
                <template v-if="consumeOperForm.priceType == 7">
                    <el-form-item :label="$t('desk.order_goodsType')+':'">
                        <el-select
                            v-model="consumeOperForm.damageTypeId"
                            @change="damageTypeIdChange"
                        >
                            <el-option
                                v-for="item in hoteldamagetypeList"
                                :key="item.id"
                                :label="item.name"
                                :value="item.id"
                            >
                    ...
                    <el-form-item :label="$t('desk.order_goodsName')+':'">
                        <el-select
                            v-model="consumeOperForm.damageId"
                            @change="getDdamageInfo"
                        >
                            <el-option
                                v-for="item in hoteldamageList"
                                :key="item.id"
                                :label="item.name"
                                :value="item.id"
                            >
                        ...
                        &nbsp;&nbsp;&nbsp;&nbsp;
                        <el-input-number
                            @change="getDamagePrice"
                            :disabled="!consumeOperForm.damageId"
                            v-model="consumeOperForm.damageCount"
                            :min="1"
                            label=""
                        ></el-input-number>
                    </el-form-item>
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="consume_oper(1, 'entry')">{{
                    $t("desk.enterAccount")
                }}</el-button>
            </div>
        </el-dialog>
        ...
        >
            ...
            >
                ...
                <el-form-item :label="$t('desk.customer_buyerUnit') + ':'" class="" prop="creditName">
                    <el-select
                        v-model="consumeOperForm.enterId"
                        filterable
                        remote
                        reserve-keyword
            ...
            <div slot="footer" class="dialog-footer">
                <el-button @click="onAccountShow = false">{{
                    $t("commons.close")
                }}</el-button>
                <el-button
                    type="primary"
                    @click="consume_oper(2, 'onAccount')"
        ...
        <el-dialog
            top="0"
            :title="$t('desk.order_goTie')"
            :visible.sync="knotShow"
            width="500px"
        >
            <el-form
                :model="consumeOperForm"
    ...
    <el-dialog top='0' :title="$t('desk.customer_rich')" :visible.sync="destructionShow"width="800px">
        <el-form :model="consumeOperForm" ref="destruction" :rules="rules" size="mini" label-width="100px" >
			...
			<el-row v-else>
                ...
                <el-form-item
                   :label="$t('desk.order_completelyPrice')+':'"
                    prop="consumePrice"
                    v-if="consumeOperForm.priceType == 10"
                >
                    <el-input
                        class="width200"
                        type="text"
            ...
            <div slot="footer" class="dialog-footer">
                <el-button @click="destructionShow = false">{{
                    $t("commons.cancel")
                }}</el-button>
                <el-button
                    type="primary"
                    @click="consume_oper(3, 'destruction')"
                    >{{ $t("commons.confirm") }}</el-button
                >
            </div>
        </el-dialog>
        ...
        <someAccounts
            ...
            @get_consume_order_list="consume_order_list"
