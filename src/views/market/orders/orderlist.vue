            ...
            >
                ...
                <el-table-column label="操作" width="240">
                    <template slot-scope="{row}">
                        ...
                        <el-button type="text" size="mini" v-if="row.orderType == 1 || row.orderType == 2" @click="popClick(row, 'detail')">结账</el-button>
                        <el-button type="text" size="mini" @click="popClick(row, 'invoicing')">开发票</el-button>
                        <el-button type="text" size="mini" v-if="row.orderType == 3 || row.orderType == 4 || row.orderType == 5" @click="popClick(row, 'unCheckOut')">撤销退房</el-button>
...
export default {
    ...
    methods: {
        ...
        popClick(item, type) {
            if (type == 'detail') { //跳转至订单详情
                this.$router.push("/orderdetail?id=" + item.id);
            } else if (type == 'zoujie') { //调用走结接口（这里实在不知道走结怎么翻译）

            }  else if (type == 'unCheckOut') { //调用撤销退房接口

            } else if (type == 'invoicing') { //开发票
                this.$refs.invoicing.init(item.id);
            }
        },
        handelDetail(item) {
