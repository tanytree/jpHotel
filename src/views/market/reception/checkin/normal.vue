 ...
 * @LastEditTime: 2020-10-14 16:56:18
      ...
      >
        ...
        <el-form-item label="入住类型" prop="checkinType">
          <el-select v-model="checkInForm.checkinType">
            <el-option
              :value="key"
              v-for="(item, key, index) of $t('commons.checkinType')"
              :label="item"
              :key="index"
            ></el-option>
          </el-select>
      ...
      <div class="roomMsg">
        <div class="left">
          <el-form inline size="small">
            <el-form-item>
              <el-radio-group
                v-model="getRoomsForm.changeType"
                @change="getDataList"
              >
                ...
                <div class="row">
                  ...
                  <div>
                    <el-input
                      ...
                      class="num"
                      v-model="v.price"
                      v-if="getRoomsForm.changeType == 1"
                    ></el-input>
          ...
          <el-form inline size="small">
            <el-form-item>
              ...
              <el-button
                ...
                "
                ><i v-loading="liveLoading"></i>添加入住人</el-button
              >
              <el-button
                @click="live_in_person_list"
                v-if="
...
>>>>>>> Stashed changes
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="liveInPersonCancel">取消</el-button>
        <el-button size="small" type="primary" @click="liveInPersonShow = false"
          >确定</el-button
        >
      </span>
    </el-dialog>
...
=======
            window.setTimeout(() => {
              if (operCheckinType == "a1" || operCheckinType == "a2") {
                ...
                this.$router.push(
                  "/bookingDetail?id=" +
                    (data.checkInReserveId || data.checkinId)
                );
              }
            }, 2000);
          } else if (type == 3) {
...
>>>>>>> Stashed changes
    ...
    db_row_houses() {
      if (
        this.rowRoomCurrentItem.roomsArr.length > this.rowRoomCurrentItem.num
      ) {
    ...
    live_in_person_list() {
      console.log("添加入住人");
      console.log(this.waitingRoom);
      let waitingRoom2 = this.$F.deepClone(this.waitingRoom); //克隆  this.waitingRoom
      this.liveData = [];
      if (
        this.checkInForm.checkInRoomJson.length > 0 &&
        this.checkInForm.checkInRoomJson[0].personList.length > 0
      ) {
        this.checkInForm.checkInRoomJson.forEach((room, index) => {
          this.$F.merge(room, room.personList[0]);
          room.personList.splice(0, 1);
