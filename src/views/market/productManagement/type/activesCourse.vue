            ...
            <div>
              ...
              <div class="specVal">
                <span class="valTit">文件</span>
                 <div style="margin-right:10px" v-if="item.fileName">{{item.fileName}}</div>
                <el-upload
                  class="upload-demo"
                  :action="fileAction"
                  accept=".mp4"
                  ...
                  :on-success="(res, file, fileList) => mainPicSuccess(res, file, fileList,'course', index)"
                >
                  <el-button v-if="!item.fileName" type="primary" class="defaultBtn" >上传课程</el-button>
                  <el-button v-if="item.fileName" type="primary" class="defaultBtn" >更新课程</el-button>
...
export default {
  ...
  methods: {
    ...
    //上传成功回调
    mainPicSuccess: function(res, file, fileList, type, index) {
      if (res.code == 200) {
        if (type) {
            if (type == 'teachers') {
                this.form.teachers[index].headimgurl = res.data;
            } else
                this.form.arrangs[index].fileName = res.data;
        } else {
    ...
    uploadSectionFile(res, file, fileList, type, index) {
      let fd = new FormData();
      ...
      axios
        .post(
          "http://xxyweeds.top:8895/edt/upload/upload_video",
          { files: res.file },
        ...
        .then(
          res => {
            ...
            this.form.arrangs[index].fileName = res.name;
          },
          res => {
            console.log(res);
          }
