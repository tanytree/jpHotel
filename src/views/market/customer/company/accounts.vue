...
<template>
    ...
    <div class="boss-index">
            ...
            <el-table ref="multipleTable" v-loading="loading" :data="tableData" height="100%" header-row-class-name="default" size="small">
                ...
                <el-table-column :label="$t('desk.customer_arage')" show-overflow-tooltip width="120px">
                    <template slot-scope="{ row }">
                        <div>
                            <span style="color: #f11717">{{ $t("desk.serve_open") }}</span>{{ row.startTime }}</div>
                        <div>
        ...
        <!-- 查看入账明细dialog -->
        <el-dialog :title="$t('desk.customer_lookBuyDetail')" v-if="advanceDialog" :visible.sync="advanceDialog"
                   width="1160px" top="0">
            <div class="flexBox">
                <div>
                    <span>{{ $t("desk.customer_totalCreditAmount") + ":" }}{{ totalConsumerPrice }}</span>
                    <span style="margin-left: 10px">{{ $t("desk.customer_cardRecords") + ":" }}{{
                            buyTable.length
                        }}{{ $t("desk.customer_article") }}</span>
                </div>
                <!--        <el-button type="primary">导出EXCEL</el-button>-->
            </div>
            <el-table ref="multipleTable" v-loading="loading" :data="buyTable" height="100%"
                      header-row-class-name="default" size="small">
                <el-table-column prop="createTime" :label="$t('desk.customer_spendTime')" show-overflow-tooltip
        ...
        <!-- 入账 dialog -->
        <el-dialog :title="$t('desk.enterAccount')" v-if="settlementDialog" :visible.sync="settlementDialog"
                   width="600px" top="0">
            ...
            <el-form size="small" :model="enterForm" ref="enterForm" style="margin-top: 15px"
                     label-width="90px" :rules="enterRules">
                ...
                <el-form-item label="汇款时间">
                    <el-date-picker
                        v-model="enterForm.remittanceTime"
                        type="datetime"
