...
const mixin= {
    ...
    methods: {
               ...
               let sum = 0 //合计
                list.forEach(element => {
                   ...
                   // console.log(list[i].goods.priceModel)
                    ...
                    if(element.taxStatus == 2 && element.seviceStatus == 1){
                        if(outFlag){
        ...
        getTaxInfo(tax,list,outFlag,endTime,info){
            ...
            if(list && list.length > 0 && tax){
                ...
                // for(let i in list){
                    ...
                    //         let f = 1.00 + consumeTax
                    //         let s = (element.totalPrice / f) * servicePrice
                    //         taxInFee += this.getTaxInOff2(tax,element.totalPrice,s)
                    //     }
                    // }
        ...
        //包含服务费和包含税
		getTaxInOff2(tax,price,service){
			let per =price * (1 - 1/(1.00 + tax) )
			let taxInFee = parseFloat(per).toFixed(0)
			let t = Math.round(taxInFee *1)/1
			// console.log(tax)
			// console.log(price)
