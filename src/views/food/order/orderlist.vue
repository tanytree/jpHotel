  ...
  <div>
    <div>
      ...
      <div v-loading="loading">
        <el-table class="margin-t-20" header-row-class-name="default" size="small" :data="tableData" tooltip-effect="dark" style="width: 100%">
          <el-table-column prop="dishesNum" :label="$t('food.common.order_num')">
          </el-table-column>

          <el-table-column prop="deskNum" :label="$t('food.common.deskNum')">
          </el-table-column>
          <el-table-column prop="createTime" :label="$t('food.common.create_time')">
          </el-table-column>
          <el-table-column :label="$t('food.common.food_detail')">
          ...
          <el-table-column width="200" :label="$t('commons.operating')">
            <template slot-scope="scope">
    ...
    <el-dialog top="0" :title="$t('food.orderTitle.'+ dialogType)" :width="is_add ? '45%' :'70%'" :visible.sync="dialogShow" :close-on-click-modal="false" :close-on-press-escape="false" @close="closeDialog">
    ...
    <el-dialog top="0" :title="$t('food.orderTitle.1')" width="700px" :visible.sync="dialogShows" :close-on-click-modal="false" :close-on-press-escape="false" @close="closeDialog">
      <div class="detailPanel">
        ...
        <el-table class="margin-t-10 " :data="detail.orderSubList" border header-row-class-name="default" size="small">
  ...
  methods: {
      ...
      for (let i in list) {
        dishesJson.push({
          ...
          totalPrice: parseFloat(
            parseFloat(list[i].price) * parseFloat(list[i].count)
          ).toFixed(2),
    ...
    getTotalPrice(list) {
      if (list.length > 0) {
        ...
        for (let i in list) {
          sum +=
            parseFloat(list[i].unitPrice) * parseFloat(list[i].dishesCount);
        }
    ...
    cancleOrder(data) {
      ...
      params.storesNum = this.storesNum;
      this.$confirm(
        this.$t("food.common.cancel_confirm_order_tips"),
        this.$t("food.common.tip"),
        {
        ...
        .then(() => {
          this.$F.doRequest(
            this,
            "/pms/dishes/dishes_order_cancle",
            params,
            (res) => {
