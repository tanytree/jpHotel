			...
			<el-row :gutter="20">
				<el-row>
					<el-form class="demo-form-inline" inline size="small">
						...
						<el-form-item>
							<el-button type="primary" style="width: 100px;" size="mini" @click="get_hotel_rule_member_price_list">{{$t('commons.queryBtn')}}</el-button>
						</el-form-item>
					</el-form>
				</el-row>
					...
					<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" :header-cell-style="{background:'#F7F7F7',color:'#1E1E1E'}">
						...
						<el-table-column :label="$t('commons.operating')" width="200">
							<template slot-scope="scope">
								<el-button type="text" size="small" @click="popup('sit', scope.row)">{{$t('manager.ps_setUp')}}</el-button>
							</template>
						</el-table-column>
					</el-table>
					<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="ruleForm_r.pageIndex"
					 :page-sizes="[10, 20, 50, 100]" :page-size="ruleForm_r.pageSize" :total="ruleForm_r.totalSize" layout="total, sizes, prev, pager, next, jumper"></el-pagination>
				</div>
			</el-row>
		</el-row>

		<!-- 设置 -->
		<el-dialog top="0" :title="$t('manager.ps_setUp')" :visible.sync="dialogsit" :close-on-click-modal="false">
			<el-row :gutter="20">
			...
			<span slot="footer" class="dialog-footer">
				<el-button @click="dialogsit = false">{{$t('commons.cancel')}}</el-button>
				<el-button type="primary" @click="saveInfo">{{$t('commons.determine')}}</el-button>
			</span>
		</el-dialog>
	</div>
</template>
