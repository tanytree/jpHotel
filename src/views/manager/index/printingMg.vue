        ...
        methods: {
            ...
            submit(type) {
                ...
                if(type == 'params') {
                    ...
                    this.printData.map(item => {
                        const obj = {id: item.id, name: item.name, remark: item.remark, number: item.number, format: item.format,depositFlag: item.depositFlag};
                        params.push(obj)
                    })
                }
                this.$F.doRequest(this, "/pms/documentsparams/batchedit", {params: JSON.stringify(params)}, (res) => {
