<template>
  <div class="boss-index">
      ...
      <el-form class="term line demo-form-inline" v-model="form" inline size="small">
        ...
        <el-form-item class="form-inline-flex">
          <el-button
            ...
            @click="shelf"
            class="submit"
          >{{$t('manager.grsl_goodsShelves')}}</el-button>
        </el-form-item>
      </el-form>
        ...
        >
          ...
          <el-table-column :label="$t('commons.operating')" width="150">
            <template slot-scope="scope">
              ...
              >{{$t('manager.grsl_shelves')}}</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="block">
    ...
    >
      ...
      >
        ...
        <el-table-column :label="$t('manager.grsl_allowedToRoom')">
          <template
            slot-scope="scope"
          >{{scope.row.allowRoom == 1 ? $t('manager.hk_yes') : $t('manager.hk_no')}}</template>
        </el-table-column>
        <el-table-column :label="$t('manager.grsl_allowerToUnit')">
          <template
            slot-scope="scope"
          >{{scope.row.allowEnter == 1 ? $t('manager.hk_yes') : $t('manager.hk_no')}}</template>
        </el-table-column>
        <el-table-column :label="$t('boss.loginDetail_state')">
          <template
            slot-scope="scope"
          >{{scope.row.state == 1 ? $t('manager.grsl_toEnble') : $t('commons.disable')}}</template>
        </el-table-column>
    ...
    <el-dialog
      top="0"
      :title="$t('manager.grsl_goodsShelves')"
      :visible.sync="shelfVisible"
      width="1000px"
      :close-on-click-modal="false"
    >
