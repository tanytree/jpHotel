<template>
  <div class="boss-index">
    ...
    <!-- 售卖点管理 -->
    <el-dialog top="0" :title="$t('manager.grsl_managementSalePoint')" :visible.sync="salePointVisible" width="600px" :close-on-click-modal="false">
        ...
        <el-form :model="rowData" size="small" inline :rules="threerules" ref="priceForm" label-position="top" class="price">
          ...
          <el-form-item prop="employeePrice" :label="$t('manager.grsl_employeePrice')">
            <el-input v-model="rowData.employeePrice" class="row-width">
              <svg slot="prefix" t="1611640798495" class="svg-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3317" width="128" height="128">
                <path
  ...
  methods: {
    ...
    popup(type, row) {
      if (type == "sale") {
        ...
        this.editPointTitle = this.$t("manager.grsl_resetSalePoint");
        ...
        this.editPointVisible = true;
        this.editPointTitle = this.$t("manager.grsl_addNewPoint");
        this.pointType = true;
        this.point = {
          name: "",
          allowEnter: 1,
    ...
    search(type) {
      if (type) {
        const category = this.upshelf.category[
          this.upshelf.category.length - 1
        ];
        this.getShelfData(
          category,
          this.upshelf.name,
          this.upshelf.categoryType
        );
      } else {
        this.initData(
          this.pageForm,
