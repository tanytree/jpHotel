          ...
          <span class="custom-tree-node" slot-scope="{node, data}">
            ...
            <span>
              ...
              <el-button
                class="btn-text"
                type="text"
                v-if="data.categoryLevel == 2"
                size="mini"
                @click="() => addThird(node, data)"
                @click.stop
              >{{$t('manager.grsl_addThridGroup')}}</el-button>
              <el-button
                class="btn-text"
                type="text"
                size="mini"
    ...
    >
      <el-form label-position="right" label-width="120px" size="medium">
        <el-form-item :label="$t('manager.grsl_firstGroupName')+':'">
          <el-input
            :disabled="category.categoryLevel==2 || category.categoryLevel==3 ? true: false"
            v-model="category.first"
          ></el-input>
        </el-form-item>
        <el-form-item
          v-if="category.categoryLevel==2 || category.categoryLevel==3"
          :label="$t('manager.grsl_secondGroupName')+':'"
        >
          <el-input :disabled="category.categoryLevel==3 ? true: false" v-model="category.second"></el-input>
        </el-form-item>
        <el-form-item
          v-if="category.categoryLevel==3"
          :label="$t('manager.grsl_thridGroupName')+':'"
        >
...
export default {
  ...
  computed: {
    ...
    deleteContent: {
      get() {
        return this.$t("boss.grsl_deleteContent");
      },
      set() {},
    },
  },
