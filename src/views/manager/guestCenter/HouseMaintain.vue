  ...
  <el-row class="boss-index">
    <el-row v-if="rili_show" class="boss-index">
      <el-tabs class="tabCenter" v-model="active_tag" v-if="tab_show">
        ...
        <el-tab-pane :label="$t('manager.hk_guestRoom')" name="one">
          <el-container direction="vertical" class="boss-index">
            ...
            <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="form.pageIndex" :page-size="form.pageSize" :total="form.totalSize" layout="total, prev, pager, next, jumper"></el-pagination>
          </el-container>
        </el-tab-pane>
        <!-- 会议厅房型 -->
      ...
      <div class="infinite-list-wrappers flex_column" v-if="!tab_show">
        <div class="back">
          <el-page-header @back="back" :content="ifmeeting?$t('manager.hk_newAddRoomTypeA'):(isRoomAdd?$t('manager.hk_newAddRoomType'):$t('manager.hk_addRoomType'))"></el-page-header>
        </div>
        <div class="infinite">
          <!-- 基本信息 -->
          ...
          <el-form size="small" label-width="120px">
            <el-form-item :label="$t('manager.hk_roomImg')+':'">
              <el-upload list-type="picture-card" action="aa" ref="upload" :file-list="files" :auto-upload="false" multiple accept="image/png, image/gif, image/jpg, image/jpeg">
            ...
            <el-form-item :label="$t('boss.loginDetail_note')+':'">
              <el-input type="textarea" :rows="4" style="width: 800px" resize="none" :placeholder="$t('manager.hk_placeEnterContentD')" v-model="ruleForm.remark"></el-input>
            </el-form-item>
          </el-form>
        </div>
        <div class="footer">
    ...
    addHouse(type, value) {
      switch (type) {
        ...
        case "change":
          ...
          this.ruleForm = value;
          this.files = [];
          let arr = (value.personPrice || value.marketPrice)
            .toString()
            .split(",");
          let obj = {};
          //
      ...
      if (this.ruleForm.roomType == 1) {
        if (this.ruleForm_sit.length == 0) {
          ...
          if (this.ruleForm.personPrice[0] == ",") {
            this.ruleForm.personPrice = this.ruleForm.personPrice.substring(
              1,
              this.ruleForm.personPrice.length
            );
          }
        }
      } else {
            ...
            if (
              ...
              arry.forEach((item, i) => {
                ...
                if (value.mealBreakfastObject) {
                  allP =
                    Number(item) +
                    Number(value.mealBreakfastObject.mealPrice) * (i + 1);
                }
                if (value.mealDinnerObject) {
                  allP =
                    Number(item) +
                    Number(value.mealDinnerObject.mealPrice) * (i + 1);
                }
                ...
                obj.pName = `${i + 1} ${this.$t(
                  "boss.add_peopleLivePrice"
                )}${this.$t("manager.hk_livePrice")}+${this.$t(
                  "desk.side_sidePri"
                )}`;
                obj.allPrice = allP;
                arr.push(obj);
