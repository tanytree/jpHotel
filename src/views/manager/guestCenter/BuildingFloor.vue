...
<template>
  <div id="page1">
    <el-row :gutter="20" style="font-size: 14px; font-weight: bolder;">
      ...
      <el-col :span="2">{{$t('manager.hk_total')}}{{dongList.length || ''}}{{$t('manager.hk_building')}}</el-col>
    </el-row>
    ...
    <el-row :gutter="20" class="demo-form-inline" style="background-color: #e6eaed;padding: 10px 0px;">
      <el-col :span="6">
        ...
        <span style="color: #126EFF;">{{dongInfo.buildingTotal}}</span>
        {{$t('manager.hp_layer')}} {{$t('manager.hk_totalRoom')}}:
        <span style="color: #126EFF;">{{dongInfo.roomTotal}}</span>
        {{$t('manager.hk_space')}}
      </el-col>
      ...
      <el-row v-for="(value, index) in cengList" :key="index">
        <el-popover placement="right" width="400" trigger="hover" @show="showroom_list(value)">
          ...
          <el-row>
            ...
            <el-row style="padding:20px 0px 10px 0px;">{{$t('manager.hk_floorNote')+':'}} {{value.remark}}</el-row>
            <el-row v-if="!selectRedio">
              <el-input :placeholder="$t('manager.hk_placeEnterContentC')" v-model="value.building.remark"></el-input>
            </el-row>
          </el-row>
        </el-popover>
      </el-row>
    ...
    <el-dialog top="0" :title="ceng_title" :visible.sync="addCeng_show" :close-on-click-modal="false">
      <el-row :gutter="20">
        <el-form :model="ruleForm_ceng" :rules="rules" ref="ruleForm_ceng" label-width="150px">
          <el-col :span="20">
            <el-form-item :label="$t('manager.hp_floor')+':'" prop="floor">
              <el-input type="number" :placeholder="$t('manager.hk_placeEnterContentA')" v-model="ruleForm_ceng.floor"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="20">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="defineCeng('ruleForm_ceng')">{{$t('commons.determine')}}</el-button>
      </span>
    </el-dialog>
  </div>
</template>
