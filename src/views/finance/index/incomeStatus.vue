    ...
    <el-card shadow="never">
      <el-tabs v-model="activeName" @tab-click="tabChange" class="tabCenter">
        <el-tab-pane v-for="item in paneArr" :key="item.name" :label="item.label" :name="item.name">
          <div class="chart-top">
            <p>{{item.tip}} {{date.strDate}} {{$t('boss.report_toText')}} {{date.endDate}}</p>
            <el-form v-if="activeName == 'week'" size="small" :inline="true" :model="date">
              <el-form-item>
                <el-date-picker
                  type="date"
                  :placeholder="$t('manager.finance_startTime')"
                  format="yyyy-MM-dd"
                  value-format="yyyy-MM-dd"
                  v-model="date.strDate"
                ></el-date-picker>
              </el-form-item>
              <el-form-item>{{$t('boss.report_toText')}}</el-form-item>
              <el-form-item>
                <el-date-picker
                  type="date"
                  :placeholder="$t('manager.finance_endTime')"
                  format="yyyy-MM-dd"
                  value-format="yyyy-MM-dd"
  ...
  methods: {
    ...
    openChart() {
      this.$nextTick(() => {
        ...
        let type =
          this.activeName == "week" ? 1 : this.activeName == "month" ? 2 : 3;
