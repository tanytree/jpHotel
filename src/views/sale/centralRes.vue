    ...
    <div>
                    ...
                    <el-col :span="9">
                        <el-form-item :label="$t('manager.hp_hotelName')+':'">
                            <el-select v-model="centerForm.storesNum" :placeholder="$t('login.sTip')">
                                ...
                                <el-option v-for="item in storeList" :key="item.storesNum" :label="item.storesName"
                                           :value="item.storesNum"></el-option>
                            </el-select>
                        </el-form-item>
                    </el-col>
                    <el-col :span="8">
                        <el-form-item>
                            <el-button type="primary" style="margin-left: 25px" @click="onSubmit">
                                {{ $t('commons.queryBtn') }}
                            </el-button>
                            <el-button type="primary" style="margin-left: 25px" @click="innitData">
                                {{ $t('commons.resetBtn') }}
                            </el-button>
                        </el-form-item>
                    </el-col>
        ...
        <el-row>
            ...
            <el-row v-for="(item, index) in infoArray" :key="index" style="margin-top: 30px" v-loading="loading">
                ...
                <el-table :data="tableData" style="width: 100%; margin-top: 10px" :cell-style="{ verticalAlign:'top'}"
                          :header-cell-style="{ background: '#D9DDE2', color: '#606266' }" border>
                    <el-table-column :label="$t('desk.home_roomType')">
                        <template>
                            <div>{{ $t('boss.add_roomDetail') }}</div>
                            <div>{{ $t('boss.add_onlyLivePri') }}</div>
                        </template>
                    </el-table-column>
                    <el-table-column v-for="(each, index) in item.array" :key="index" :label="each.roomTypeName">
                        <template>
                            ...
                            <el-row v-if="each.personPrice && each.personPrice.split(',').length > 0">
                                <el-col v-for="(price, secondIndex) in each.personPrice.split(',')" :key="secondIndex">
                                    <div v-if="secondIndex==0"> {{ $t('boss.add_onePeople') }}：{{
                                            price|numFormate
                                        }}
                                    </div>
                                    <div v-else> {{
                                            secondIndex + 1
                                        }}{{ $t('boss.add_peopleLivePrice') }}：{{ price|numFormate }}
                                    </div>
                                </el-col>
                            </el-row>
                            <el-row v-else>1{{ $t('boss.add_peopleLivePrice') }}：{{
                                    checkedPrice(each.onePersonPrice) || checkedPrice(each.personPrice)
                                }}
                            </el-row>
                        </template>
                    </el-table-column>
                    <el-table-column :label="$t('commons.operating')" fixed="right" width="260">
                        <template>
                            <el-button :disabled="item.array.length > 0 ? false : true" size="mini" type="text"
                                       @click="handleReserve(item, 'b1')">{{ $t('boss.add_normalBook') }}
                            </el-button>
                            <el-button :disabled="item.array.length > 0 ? false : true" size="mini" type="text"
                                       @click="handleReserve(item, 'b2')">{{ $t('boss.add_dayBackBook') }}
                            </el-button>
                            <el-button :disabled="item.array.length > 0 ? false : true" size="mini" type="text"
                                       @click="handleReserve(item, 'b3')">{{ $t('boss.add_meetingBook') }}
                            </el-button>
                        </template>
                    </el-table-column>
                </el-table>
        ...
        <el-dialog top="0" :visible.sync="reserveShow" class="rowRoomDia" :title="$t('desk.rowHouseA')" width="1400px">
            <normal :selectStoresNum="selectStoresNum" :operCheckinType="operCheckinType" ref="normal" @cancel="cancel"></normal>
        </el-dialog>
    </div>
</template>
<script>
