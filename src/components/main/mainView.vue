        ...
        menulfuc(item) {
          console.log(item);
            if (item.childList && item.childList.length > 0) {
                ...
                let sessionPath = sessionStorage.pagePath, childMenu = this.menul.childList[0];
                if (this.refreshInto) {
                    this.menul.childList.forEach(child => {
                        if (child.path == sessionPath) {
                            childMenu = child;
                        }
                    })
                    this.refreshInto = false;
                }
