 ...
 * @LastEditTime: 2020-10-12 17:01:08
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="mini"
            @click="del_live_in_person(scope.row, scope.$index)"
  ...
  methods: {
    ...
    personSubmit() {
      ...
      this.liveInPersonData.forEach((item) => {
        if (!item.personList) item.personList;
        let temp = {
        ...
        this.$F.doRequest(
          ...
          (res) => {
            ...
            params = {
              ...
              personListJson: JSON.stringify(
                this.liveInPersonData[0].personList
              ),
...
=======
    ...
    del_live_in_person(item, index) {
      ...
      for (let i = 0; i < this.liveInPersonData.length; i++) {
        currentIndex = i;
        if (
          this.liveInPersonData[i].personList &&
          this.liveInPersonData[i].personList.length > 0
        ) {
          let personList = this.liveInPersonData[i].personList;
          for (let j = 0; j < personList.length; j++) {
            if (personList[j].roomId == item.roomId) {
...
>>>>>>> Stashed changes
      ...
      this.$set(
        ...
        currentIndex,
        this.liveInPersonData[currentIndex]
      );
      this.$forceUpdate();
    },
  },
