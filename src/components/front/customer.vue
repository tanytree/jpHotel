...
<template>
  <div class="customer">
    ...
    >
      ...
      <el-table-column label="状态">
        <template slot-scope="{ row }">
          <el-button
            type="text"
            v-if="row.inRoomList && row.inRoomList.personList > 0"
            >点击入住</el-button
          >
    ...
    >
      ...
      <el-table-column label="操作" width="180">
        <template slot-scope="scope">
          <el-button type="text" size="mini" @click="edit_live_in_person()"
            >编辑</el-button
          >
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    del_live_in_person(item) {
      ...
      this.$F.doRequest(
        this,
        "/pms/checkin/delete_live_in_person",
        params,
        (res) => {
          this.$message({
            message: "移除成功",
    ...
    edit_live_in_person() {
      ...
      tempArray.forEach((roomTypeObject) => {
        if (roomTypeObject.personList && roomTypeObject.personList.length > 1) {
          ...
          roomTypeObject.personList &&
          roomTypeObject.personList.length == 1
        ) {
          this.$F.merge(roomTypeObject, roomTypeObject.personList[0]);
