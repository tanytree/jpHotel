<template>
    <div class="boss-index">
        <!--薪酬设置-->
        <div class="booking flex_column" v-loading="loading">
            <el-form class="term" inline size="small">
                ...
                <el-form-item :label="$t('boss.loginDetail_department')+':'" class="margin-l">
                    <el-select v-model="searchForm.departmentId" :placeholder="$t('boss.compensation_selectDepartment')">
                ...
                <el-form-item class="margin-l">
                    <el-button type="primary" class="submit" @click="getEmployeeList">{{$t('commons.queryBtn')}}</el-button>
                </el-form-item>
            </el-form>
            <el-table ref="multipleTable" :data="setTableData" height="100%" header-row-class-name="default" size="small">
                ...
                <el-table-column :label="$t('commons.operating')" width="200">
                    <template slot-scope="{row}">
                        ...
                        >{{$t("boss.compensation_toView")}}
                        </el-button>
                        <el-button
                                type="text"
                                size="small"
                                @click="popup('set_change',row)"
                        >{{$t("boss.compensation_wageSetting")}}
                        </el-button>
                    </template>
                </el-table-column>
            </el-table>
            <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="searchForm.pageIndex" :page-size="10" layout=" prev, pager, next, jumper" :total="listTotal"></el-pagination>
        </div>
        <!-- 薪酬设置 / 查看-->
        <el-dialog :title="salary_set_title" :visible.sync="dialogsalary_set" :close-on-click-modal="false" top="0" class="salary_set">
            ...
            <div slot="footer" class="dialog-footer" v-else>
                ...
                <el-button type="primary" class="submit" @click="setSalarySubmit" v-loading="loading">{{$t('commons.determine')}}</el-button>
            </div>
        </el-dialog>
    </div>
</template>
