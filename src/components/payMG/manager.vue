	...
	<div class="boss-index">
        <div class="content">
            <el-form class="demo-form-inline" inline size="small">
                <el-form-item label="所属门店:" v-if="storesNum == $F.getHQCode()" class="margin-l">
                    <el-select v-model="payManagerForm.storesNum" placeholder="请选择状态">
                        <el-option label="全部" value="">全部</el-option>
                        <el-option :label="item.storesName" :value="item.storesNum" :key="index" v-for="(item, index) in storeList"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="员工名称:" class="margin-l">
                    <el-input v-model="payManagerForm.content" ></el-input>
                </el-form-item>
                <el-form-item label="工资月份:" class="margin-l">
                    <el-date-picker v-model="payManagerForm.payTime" type="month" placeholder="选择月" value-format="yyyy-MM"></el-date-picker>
                </el-form-item>
                <el-form-item label="员工状态:" class="margin-l">
                    <el-select v-model="payManagerForm.userStatus" placeholder="请选择状态">
                        <el-option label="全部" value="">全部</el-option>
                        <el-option :label="label" :value="value" :key="value" v-for="(label, value) in $t('commons.userStatus')"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item>
            ...
            <div class="components-edit">
                <el-table ref="multipleTable" :data="payManagerTableData" height="100%" tooltip-effect="dark" :header-cell-style="{background:'#e6eaed',color:'#1E1E1E'}">
                    <el-table-column prop="employee.position" label="职位"></el-table-column>
                    <el-table-column prop="employee.userName" label="姓名"></el-table-column>
                    <el-table-column label="员工状态">
                        <template slot-scope="scope">
                            {{$t('commons.userStatus')[scope.row.employee.userStatus || '']}}
                        </template>
                    </el-table-column>
                    <el-table-column prop="employee.worknum" label="工号"></el-table-column>
                    <el-table-column prop="payTime" label="工资月份"></el-table-column>
                    <el-table-column label="基本工资">
                        <el-table-column prop="wage.baseWage" label="基础工资"></el-table-column>
                ...
                <div class="block">
                    ...
                    <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="payManagerForm.pageIndex"
                                   :page-sizes="[100, 200, 300, 400]" :page-size="100" layout=" sizes, prev, pager, next, jumper" :total="400"></el-pagination>
                </div>
            </div>
        </div>
		<!-- 薪酬修改 / 查看-->
		<el-dialog :title="salary_change_title" :visible.sync="dialogAdd" :close-on-click-modal="false" top="0">
		...
		methods: {
          ...
          getEmployeeList() {
			...
			get_salary_mg_list() {
				...
				let params = this.payManagerForm
