  ...
  <div>
    <el-row :gutter="20" v-if="tab1_show">
      <el-row style="padding: 10px 15px;">
        <el-form class="demo-form-inline" inline size="small">
          ...
          <el-form-item :label="$t('manager.grsl_selectTime')+':'" style="display: flex;">
            <el-date-picker v-model="search_d.strategyTime" value-format="yyyy-MM-dd" align="right" type="date" :placeholder="$t('commons.selectDate')" :picker-options="pickerOptions"></el-date-picker>

          </el-form-item>
          <el-form-item>
            <el-col>
              <el-button type="text" @click="beforeTap" style="border-bottom: 2rpx solid #409EFF; margin-left: 20rpx;">&lt;&lt; {{$t('manager.add_lsat15')}}</el-button>
            </el-col>
          </el-form-item>
          <el-form-item>
            <el-col>
      ...
      <div class="components-edit member-price">
        <el-table :data="roomType" style="width: 100%;margin-bottom: 20px;" row-key="id2" default-expand-all header-row-class-name="default">
    ...
    <!-- ===================批量调价==================================== -->
    <el-dialog top="0" :title="$t('manager.ps_bulkPrice')" :visible.sync="PieDialog" width="80%">
      <div>
        <el-form :model="ruleForm_Pie" :rules="rules" ref="ruleForm_Pie" label-width="100px">
          ...
          <el-row>
            ...
            <el-form-item :label="$t('manager.ps_selectWeekA')">
              ...
              <el-checkbox-group v-model="ruleForm_Pie.weeks" @change='checkboxChange'>
      ...
      <el-table :data="ruleForm_Pie.roomStrategyJson" header-row-class-name="default">
        ...
        <el-table-column :label="$t('manager.add_peoAsit')" v-if="ruleForm.roomType == 1">
          <template>
            <div>
    ...
    <el-dialog top="0" :title="$t('manager.ps_resetRoomPrice')" :visible.sync="editPriceDialog" :close-on-click-modal="false" width="80%" class="editPriceDialog">
      ...
      <el-table ref="multipleTable" :data="editPriceForm.roomStrategyJson" tooltip-effect="dark" default-expand-all header-row-class-name="default">
...
export default {
  ...
  methods: {
    checkboxChange(){
      if(this.ruleForm_Pie.weeks.length<7){
        this.checkAll= false;
      }else{
        this.checkAll = true;
      }
        ...
        if (newArray && newArray.length > 0) {
          ...
          priseList.forEach((c, d) => {
            if (topIndex == 0) {
              ...
              result +=
                `${d + 1}` +
                this.$t("manager.add_peoplePrice") +
                Number(priseList[d]) +
                "<br/>";
            } else if (topIndex == 1) {
              //住宿+早
              result +=
                ...
                Number(
                  Number(priseList[d]) +
                    Number(this.mealBreakfastObject.mealPrice || 0)
                ) +
                "<br/>";
            } else if (topIndex == 2) {
              //住宿+晚餐
              result +=
                ...
                Number(
                  Number(priseList[d]) +
                    Number(this.mealDinnerObject.mealPrice || 0)
                ) +
                "<br/>";
            } else if (topIndex == 3) {
              //住宿+晚餐+早餐
    ...
    handleWeekDayChange(value) {
      ...
      if (value.length == 0) this.ruleForm_Pie.weeks = [];
                  ...
                  arry.forEach((c, d) => {
                    if (index == 0) {
                      ...
                      stay +=
                        `${d + 1}` +
                        this.$t("manager.add_peoplePrice") +
                        this.$F.numFormate(Number(arr[d])) +
                        "<br/>";
                    } else if (index == 1) {
                      //住宿+早
                      stayX +=
                        ...
                        this.$F.numFormate(
                          Number(
                            Number(arr[d]) +
                              Number(
                                value.roomType.mealBreakfastObject.mealPrice ||
                                  0
                              )
                          )
                        ) +
